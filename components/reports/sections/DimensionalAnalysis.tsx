...
export function DimensionalAnalysis({ assessment, className = '' }: DimensionalAnalysisProps) {
  ...
  return (
    <section
      ...
      <h2 id="dimensional-analysis-heading" className="text-2xl font-bold text-foreground mb-6">
        Dimensional Analysis
      </h2>

      {/* Radar Chart - Pillar Overview */}
      <div className="mb-12">
        <h3 className="text-xl font-semibold text-foreground mb-4">Pillar Overview</h3>
      ...
      {assessment.pillars.map((pillar, pillarIdx) => (
        <div key={pillarIdx} className="mb-10 last:mb-0">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-xl font-semibold text-foreground">{pillar.pillar}</h3>
          ...
          <div className="space-y-6">
            {pillar.dimensions.map((dimension, dimIdx) => (
              <div key={dimIdx} className="border-l-2 border-border pl-6">
                ...
                {dimension.keyFindings.length > 0 && (
                  <div className="mb-4">
                    <h5 className="text-sm font-semibold text-foreground mb-2">Key Findings</h5>
                    <ul className="space-y-1">
                      {dimension.keyFindings.map((finding, findingIdx) => (
                        <li key={findingIdx} className="flex items-start gap-2 text-sm text-muted-foreground">
                  ...
                  <div className="mt-4 pl-4 border-l-2 border-border">
                    ...
                    <div className="space-y-2">
                      {dimension.supportingQuotes.slice(0, 2).map((quote, quoteIdx) => (
                        <p key={quoteIdx} className="text-sm italic text-muted-foreground">
                          "{quote}"
                        </p>
                      ))}
                    </div>
